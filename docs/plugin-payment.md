...
4. 直接打开独立的爱发电支付窗口，并复用之前的爱发电登录状态。
5. 每 3 秒向 Server 查询一次订单状态，最长持续约 30 分钟。
6. 确认支付成功后关闭支付窗口，并调用插件传入的成功回调。

支付由主窗口中的插件发起时，支付结束后恢复主窗口；由分离插件窗口发起时，只恢复对应的分离窗口，不会额外唤起主窗口。
